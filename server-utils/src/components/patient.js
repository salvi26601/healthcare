...
class Patient extends Component {
  ...
  componentDidMount() {
    ...
    if (healthRecord) this.loadPatient(healthRecord);
  ...
  async grantAccess() {
    ...
    if (this.state.doctorId) {
      let res = await healthRecord.grantAccessToDoctor.sendTransaction(
        this.state.doctorId,
  ...
  onTextChange(type, e) {
    if (e && e.target) this.setState({ [type]: e.target.value });
  }
    ...
    onChange: (info) => {
      ...
      if (status === "done") {
        if (info.file.response) {
          ...
          } else message.error("file upload unsuccessful");
        }
      ...
      <div>
          ...
          <Col className="col-3" span={6}>
            <Card bordered={true}>
              <Input
                ...
                onChange={this.onTextChange.bind(this, "secret")}
                size="small"
                placeholder="One Time Secret"
              />
              <Dragger
                {...this.fileProps}
                disabled={this.state.secret ? false : true}
              >
        ...
        <Row gutter={16}>
          <Collapse className="folderTab" defaultActiveKey={["1"]}>
            <Panel header={<FolderOutlined />} key="1">
              {files.map((fhash, i) => {
                let filename = this.state.filesInfo[i]
                  ? this.state.filesInfo[i][0]
                  : null;
